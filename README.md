...
As for the real experimental data, we shared the original movies as obtained experimentally as `.tif` files on [Dryad](https://doi.org/10.5061/dryad.5x69p8d8g) and included a brief description of the contents. Additionally, we included all relevant experimental conditions and procedures that are sufficient to generate a similar dataset. We identified these set of parameters as: `1)` data type (relevant to the type of experimental platform used), `2)` example (file number of the shared example), `3)` imaging modality (brightfield or phase contrast), `4)` post-seeding imaging (time at which images were taken following cell seeding), `5)` microscope, `6)` camera, `7)` imaging magnification, `8)` imaging frequency, `9)` pillar stiffness (a parameter relevant to the experimental platform), `10)` tissue preparation protocol, `11)` testbeds fabrication (description of fabricating the experimental platform), and `12)` additional details (details specific to the experimental platform). We summarized this information in a `PDF` file and provided a machine readable `.csv` version as well. 

An important aspect of data curation is data organization. We expend a lot of time and effort on organizing the data in a meaningful way. A straightforward example is our [small dataset](https://doi.org/10.5061/dryad.5x69p8d8g) of 24 examples, where we separated the examples based on the most general common category among them, the data type. As such, the [dataset](https://doi.org/10.5061/dryad.5x69p8d8g) is organized into 3 different folders, ``Type1``, ``Type2``, and ``Type3``, with individual files within each folder given consecutive numerical tags. We found this approach to be more practical than having all files in one folder with the file name reflecting the data type, especially for users with limited coding experience. Another consideration is to include leading zeros for the numbers in the file names so that their ascending order matches with the alphabetical order.

A more challenging example of data organization can be found in our larger datasets of [732](https://doi.org/10.5061/dryad.sqv9s4nbg) examples of ``Type1`` and [808](https://doi.org/10.5061/dryad.3r2280gqd) examples of ``Type2`` respectively. Although all files within each dataset share the same data type, they are fundamentally different in terms of experimental conditions as well as the batch of stem cell-derived cardiomyocytes used. As such, our data organization has two levels of hierarchy: ``condition`` at the top level and ``batch`` at the second level. Although separated into different folders, we adopted an incremental numerical file naming starting at `00000` in `batch_000` folder within `condition_000` folder and ending at `00731` in `batch_000` folder within `condition_030` folder for the first dataset and at `00807` in `batch_000` folder within `condition_020` folder for the second dataset. We found this approach to be more convenient for organizing and sharing the corresponding metadata where the file number is sufficient to identify the specific example. 

To keep track of comprehensive metadata efficiently, we included this information in 3 separate files: `1)` a common experimental metadata file including details that are common across all experimental conditions, `Type#_common_experimental_metadata`, `2)` a description of the unique numerically-coded experimental condition for each set, `Type#_experimental_conditions`, and `3)` an example specific metadata file (`metadata_type#`) that includes the following information -- `file_name`, `experimental_conditions`, `batch`, `comments`, and `included_in_plot`. The last two columns correspond to our implementation of the datasets to our pillar tracking software, [MicroBundlePillarTack](https://github.com/HibaKob/MicroBundlePillarTrack), where `comments` might include `0` for `No comment`, or `6` for `Tracked outputs are noisy` and `included_in_plot` might include `0` for `Not included` and `1` for `Included` in the summary plot that we generated for our [manuscript figure](https://doi.org/10.48550/arXiv.2405.1109)(Figure 1). Of note, we share, as a separate file, a legend for the numerical comments.     

When developing the above data organization structure, we made sure to incorporate insights from all individuals who generated the experimental data to make sure that it truthfully reflects the data at hand. For implementing the organization plan, we adopted the following practices: 

* retain the original files and their original organization -- we copied the files into their new directories   
* automate data organization -- we wrote python scripts that locate the original files, categorize them based on user-defined conditions, and finally, copy them to their new paths based on these conditions
* automate folder size control and compression -- we wrote a python script that identifies folders whose size exceeds 10 Gegebytes (GB), splits them into 2 or more folders, and compresses the highest level folders (i.e. individual condition folders). Our goal here is to make the data amenable to download by users with suboptimal internet speeds
* automate metadata generation when possible -- we wrote python scripts that generate the example-specific metadata based on user-defined conditions. We found this part of documentation to be the trickiest as some aspects could not be fully automated. For example, the `comments` column which describes why each example failed or resulted in suboptimal outputs with [MicroBundlePillarTack](https://github.com/HibaKob/MicroBundlePillarTrack) required some manual labeling. One example is that we could automatically detect when a mask could not be generated entirely while generating suboptimal masks could not be detected in all cases. 
* document file renaming -- we created a `.csv` file that maps the new file path to the original one for our reference  
